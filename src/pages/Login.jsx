      ...
      <div className="card" style={{width:'100%', maxWidth:460}}>
        ...
        <form onSubmit={onSubmit} style={{display:'grid',gap:10}}>
          ...
          <div style={{position:'relative'}}>
            <input placeholder="Password" type={showPass ? 'text':'password'} value={password} onChange={e=>setPassword(e.target.value)} required style={{paddingRight:84}} />
            <button type="button" className="action-btn" onClick={()=> setShowPass(v=>!v)} style={{position:'absolute',right:8,top:8}}>{showPass ? 'Hide' : 'Show'}</button>
          </div>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
            ...
            <Link to="/forgot" className="desc">Forgot Password</Link>
          </div>
